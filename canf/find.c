/*
	...
	Перебор адресов с отправкой данных. Попытки найти коды посылок, которые не известны.
...
uint16_t addr_ex[] = { 
	0x000, 0x100, 0x101, 0x110, 0x111, 0x212, 0x215, 0x218, 0x308, 0x312, 0x325, 0x415, 0x424, 0x445, 0x447, 0x584, 0x608 }; 

int is_addr_in_list(int addr) {
	int i=0;
	while (i < sizeof(addr_ex)/sizeof(uint16_t)) {
		if (addr_ex[i] == addr) {
			return 1;
		}
		i++;
...
int ev_can(int param) {
	...
	sprintf(data, "t");
	offset = 1;
	sprintf(&data[offset], "%03x", addr);
	offset += 3;
	sprintf(&data[offset], "%1x", len);
	offset += 1;
	for (i=0; i<len; i++) {
		sprintf(&data[offset], "%02x", packet[i]);
		offset += 2;
	}
