...
/*----------------------------------------------------------------------------
  setup CAN interface.
 *----------------------------------------------------------------------------*/
int CAN_setup (uint32_t baudrate)  {
	uint32_t i;
	...
	for ( i = 0; i < _MSG_OBJ_MAX; i++ ) {
		LPC_CAN->IF1_CMDMSK = _CMDMASK_WR | _CMDMASK_MASK | _CMDMASK_ARB | 
			_CMDMASK_CTRL | _CMDMASK_DATAA | _CMDMASK_DATAB;
		LPC_CAN->IF1_MCTRL  = 0;
		LPC_CAN->IF1_MSK1   = 0;
		LPC_CAN->IF1_MSK2   = 0;
		LPC_CAN->IF1_ARB1   = 0;
